...
def whole_to_percent(value):
    if 0 <= value <= 1:
        ...
        logger.error(f"Input value must be in the range [0, 1]")
...
async def parse_gribs(source_data_dir, output_directory, temp_directory):
    try:
        ...
        for file in filenames:
            ...
            os.remove(file)
            logger.info(f"Removing directories: {delete_directory}")
            removeDirectories(delete_directory)

            await create_db_pool()

            logger.info("Looping through parameter data and insert it to database...")
