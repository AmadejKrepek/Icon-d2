...
def get_latest_model_run_filenames(base_url):
    try:
        logger.info(f"Fetching available model run filenames...")
        response = requests.get(base_url)
        if response.status_code != 200:
            logger.error(f"Failed to fetch data from {base_url}. Status code: {response.status_code}")
            return []
        ...
        logger.info(f"Finished fetching available model run file names")
        return [latest_files[0]]
    except Exception as e:
        logger.error(f"Error while ferching available model run file names: {e}")
