...
def extract_output_names(input_filename, variable, output_directory):
    try:
        logger.info(f"Started extracting output names with input_filename: {input_filename}, variable: {variable} "
        ...
        if year_index is not None:
            ...
            logger.warning("Year not found in filename")

        # Continue with the rest of your code
        formatted_start_datetime = "_".join(
            [selected_start_year, selected_start_month, selected_start_day, selected_start_hour, selected_start_minute])
        formatted_end_datetime = "_".join(
            [selected_end_year, selected_end_month, selected_end_day, selected_end_hour, selected_end_minute])
        ...
        logger.info(f"Finished extracting output names with input_filename: {input_filename}, variable: {variable} "
                     f"and output_directory: {output_directory}")
        return output_filepath, model_run_formatted_date, selected_formatted_date, model_run_model, provider
    except Exception as e:
        logger.error(
            f"Error while extracting output names with input_filename: {input_filename}, variable: {variable} "
            f"and output_directory: {output_directory} and error: {e}")
