...
def aggregate_data(df, agg_column, agg_function):
    logger.info(f"Aggregating data with parameter: {agg_column} and agg_function: {agg_function}")
    if agg_function == 'sum':
...
def create_aggregates(csv_file, output_folder, configuration_file):
    logger.info(f"Started creating aggregates")
    ...
    if year_index is not None:
        ...
        if platform.system() == 'Windows':
            logger.info("This is a Windows operating system.")
            path_parts = csv_file.split('\\')
        elif platform.system() == 'Linux':
            logger.info("This is a Linux operating system.")
            path_parts = csv_file.split('/')
        else:
        ...
        if path_parts is None:
            logger.info(f"Path parts not found. Aggregates.py")
            sys.exit(1)
        ...
        if provider_name is not None and model_name is not None:
            logger.info(f"Value 1: {provider_name}")
            logger.info(f"Value 2: {model_name}")
        else:
            logger.warning("Value1 and/or Value2 not found in the file path.")
