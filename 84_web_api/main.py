...
def get_weather():
    ...
    cached_data = get_last_records(parameter, day, agg, start_date, cache)
    if cached_data:
...
if __name__ == '__main__':
    ...
    for parameter in [param for param in parameters]:
            for agg in ["max"]:  # Aggregation values
                for day in range(1, 2):  # Days from 1 to 3 for icon_d2
                    start_date = "2024-02-08 18:00:00"  # Your specific start_date
                    #background_thread = threading.Thread(target=run_background_task,
                                                         #args=(parameter, day, agg, start_date, cache))
